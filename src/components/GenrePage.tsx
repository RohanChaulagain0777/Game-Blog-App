...
type Props = {
  title: string;        // displayed heading e.g. "ACTION"
  queryKey: string;     // unique cache key e.g. "actionGames"
  apiParam: string;     // the RAWG API param e.g. "genres=action" or "tags=horror"
};

const fetchGames = async (apiParam: string, page: number) => {
  const response = await fetch(
    `https://api.rawg.io/api/games?key=${import.meta.env.VITE_API_KEY}&${apiParam}&page=${page}&page_size=20`
  );
  if (!response.ok) throw new Error(`Failed to fetch ${apiParam} games`);
  return response.json();
};

const GenrePage = ({ title, queryKey, apiParam }: Props) => {
  const [page, setPage] = useState(1);

  const { data, isLoading, error } = useQuery({
    queryKey: [queryKey, page],
    queryFn: () => fetchGames(apiParam, page),
    placeholderData: keepPreviousData,
  });

  if (error) return <p className="text-center mt-20 text-red-500">Error fetching {title} games.</p>;

  const games: Game[] = data?.results ?? [];

  return (
    <div className="flex justify-center items-center gap-2 flex-col">
      <h1 className="font-extrabold text-5xl text-center my-10">{title}</h1>
