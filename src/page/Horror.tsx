...
const fetchHorrorGames = async (page:number) =>{
  const response = await fetch(`https://api.rawg.io/api/games?key=${import.meta.env.VITE_API_KEY}&tags=horror&page=${page}&page_size=20`);
...
const Horror = () => {
  ...
  const {data, isLoading, error} = useQuery({
    queryKey: ['horrorGames',page],
    queryFn: () => fetchHorrorGames(page),
